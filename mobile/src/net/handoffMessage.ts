 ...
 * when the write failed sends them into their wallet to paste an empty clipboard, and
...
export function handoffMessage(r: { copied: boolean; opened: HandoffOutcome }): string {
  if (r.opened === "failed") {
    return r.copied
      ? "Address copied. Couldn't open Trust Wallet — paste it into your wallet by hand."
      : "Couldn't open Trust Wallet, and couldn't reach the clipboard. Copy the address above by hand.";
  }
  if (r.opened === "web") {
    // Trust Wallet didn't claim the link, which in practice means it isn't installed.
    ...
    return r.copied
      ? "Trust Wallet doesn't look installed — opened its download page. The address is copied, ready to paste once you're set up."
      : "Trust Wallet doesn't look installed — opened its download page. Copy the address above by hand.";
  }
  return r.copied
    ? "Address copied — in Trust Wallet tap Send, choose BNB Smart Chain, and paste."
    : "Opened Trust Wallet. Copy the address above, then tap Send → BNB Smart Chain.";
}
