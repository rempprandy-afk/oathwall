...
2. **Does BNB Chain include the EIP-7951 `P256VERIFY` precompile at `0x100`?** Unverified by any recon. Cheap check: `eth_call` to `0x100` with a known-good P-256 vector against the chain's RPC. If absent, the P-256 path costs a ~200-330k-gas Solidity verifier per signature instead of 6900, which likely kills it.
