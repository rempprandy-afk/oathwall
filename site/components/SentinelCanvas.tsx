...
/**
 * The hero's stage: a wireframe sentinel sphere cut clean through by a blade
 * of light — the wall. Trade packets stream in from the left; most hit the
...
export function SentinelCanvas() {
  ...
  useEffect(() => {
    ...
    const size = () => {
      ...
      const narrow = w < 900;
      cx = narrow ? w * 0.7 : w * 0.64;
      cy = narrow ? h * 0.3 : h * 0.5;
      R = narrow ? Math.min(w * 0.36, 200) : Math.min(h * 0.27, w * 0.18);
      fade = narrow ? 0.55 : 1;
    };
  ...
  return <canvas ref={ref} className="sx-canvas" aria-hidden="true" />;
}
