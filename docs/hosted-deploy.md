# Deploying hosted oathwall on Railway (testnet slice)
...
```bash
# OATHWALL_SESSION_SECRET — signs session cookies + auth nonces (>= 32 chars)
node -e "console.log(require('crypto').randomBytes(32).toString('base64url'))"
# OATHWALL_STORE_DEK — the 32-byte data-encryption key that seals session keys
...
| `OATHWALL_STORE_DEK` | the base64 DEK from step 2 |

**web only:**
| Var | Value |
|---|---|
| `OATHWALL_SESSION_SECRET` | the secret from step 2 |
| `OATHWALL_PUBLIC_ORIGIN` | the web service's public URL, e.g. `https://oathwall.up.railway.app` — auth binds signatures to it |
...
| `OATHWALL_BROWSER_TOKEN` | a fresh 32-byte secret, shared with the orchestrator |
...
| Var | Value |
|---|---|
| `OATHWALL_BROWSER_URL` | `http://oathwall-browser.railway.internal:8080` |
| `OATHWALL_BROWSER_TOKEN` | the same secret |
| `OATHWALL_DESK` *(optional)* | `1` — let the strategist research before deciding |
| `OATHWALL_DESK_MAX_STEPS` *(with the above)* | model calls per window, default 4 |

> `OATHWALL_DESK` costs several model calls per decision window instead of one.
> Raise `OATHWALL_LLM_INTERVAL_MIN` with it — the scout consumed an entire
...
| Var | Value |
|---|---|
| `OATHWALL_MARKETAUX_API_KEY` | the provider token — **orchestrator only** |
| `OATHWALL_MARKETAUX_DAILY_LIMIT` *(optional)* | requests the plan allows per day, default `100` |
| `OATHWALL_MARKETAUX_LIMIT` *(optional)* | articles one request may return, default `3` (the free tier's ceiling) |
| `OATHWALL_MARKETAUX_WINDOW_SEC` *(optional)* | refresh interval; derived from the allowance when unset |
...
> `OATHWALL_MARKETAUX_WINDOW_SEC` overrides that and is the one way to spend the
> allowance before the day ends; the desk then reports `budget-exhausted` rather
> than quietly reporting no news.
>
> `OATHWALL_MARKETAUX_LIMIT` also caps how many symbols one request may name,
...
`CMD` runs `npm run ${OATHWALL_START:-start:web}`, and `railway.json` sets no
startCommand and no healthcheck — so the only difference between the services is
the `OATHWALL_START` variable, and the HTTP-less orchestrator is never failed by a
healthcheck it can't answer.
1. **web** — new service from this repo. Leave `OATHWALL_START` unset → runs the Next dashboard. Set the web env above, then add the custom domain (`app.oathwall.dev`) and follow its DNS record.
2. **orchestrator** — a second service from the same repo. Set `OATHWALL_START=start:orchestrator`. Set the orchestrator env above. It needs **no public domain**.
