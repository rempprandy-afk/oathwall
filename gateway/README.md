...
The holder perk, done safely: a tiny server that lets **verified $OATHWALL holders**
...
oathwall is open source and self-hosted. Any key baked into the package is readable
...
```
holder ──GET /nonce──▶ single-use challenge ──sign──▶ /claim ──balanceOf──▶ token ──▶ paste into oathwall
oathwall ──Bearer token──▶ /v1/chat/completions ──your key──▶ upstream LLM ──▶ reply
```

- The upstream key lives only in `OATHWALL_GATEWAY_UPSTREAM_KEY` (env). Never logged, never sent to the client.
...
- Every request **re-checks the wallet's on-chain $OATHWALL balance** (cached 10 min, bounded size), so a holder who sells loses access.
...
- The gateway **forces its own model** server-side — the client can't run up an expensive one and never even learns which model it is (it's branded `oathwall-fast`).

Signing is **read-only proof of control** — no transaction, no private key ever leaves the holder's wallet. Fully in keeping with oathwall's non-custodial stance.
...
```bash
curl -s https://oathwall-gateway-production.up.railway.app/bitquery \
...
**`https://oathwall-gateway-production.up.railway.app`** — this is what the client
and the website actually call, and the only host with a working certificate.

`ai.oathwall.dev` is registered on the Railway service and its DNS is correct
(CNAME to the Railway target; CAA on `oathwall.dev` permits `letsencrypt.org`),
...
at it until `curl https://ai.oathwall.dev/healthz` returns `{"ok":true}`.

When it does land, three hand-written copies of the host have to move together:

| where | constant |
| --- | --- |
| `packages/core/src/token.ts` | `OATHWALL_GATEWAY_ORIGIN` — the oathwall client |
| `site/lib/gateway.ts` | `GATEWAY_ORIGIN` — the memescope page |
| `cli/bin.mjs` | the `oathwall` provider's `key` hint, shown during onboarding |
...
`OATHWALL_GATEWAY_UPSTREAM_KEY`, `OATHWALL_GATEWAY_SECRET` (32+ random bytes),
`OATHWALL_GATEWAY_RPC`, `OATHWALL_GATEWAY_BITQUERY_KEY`, and
`OATHWALL_GATEWAY_DOMAIN` set to the host you actually serve on. Point
`ai.oathwall.dev` at the Railway service.
...
### B) Vercel serverless (the `ai.oathwall.dev` domain already points at Vercel)
...
3. Add the three secrets as env vars: `OATHWALL_GATEWAY_UPSTREAM_KEY`,
   `OATHWALL_GATEWAY_SECRET` (≥32 bytes), `OATHWALL_GATEWAY_RPC` (+ optional
   `OATHWALL_GATEWAY_DOMAIN=ai.oathwall.dev`).
4. **Deploy.** Confirm against the host Vercel gives you:
   `curl https://<your-deployment>/healthz` → `{"ok":true}`. Only add
   `ai.oathwall.dev` once its certificate actually issues — see the status note
...
- `POST /v1/chat/completions` — OpenAI-compatible; `Authorization: Bearer <token>`. This is what oathwall calls.
- `GET /healthz` — liveness.

## The holder experience

1. Holder opens `https://ai.oathwall.dev/claim`, connects their wallet, signs (free).
2. Gateway checks they hold ≥ `OATHWALL_GATEWAY_MIN_TOKENS` and returns a key.
3. In oathwall → **Settings → AI provider → Oathwall AI**, they paste the key. Done — chat + the strategist now run on your dime, no third-party signup.

## Costs & limits (read before you flip it on)

You are paying for holders' inference. Protect yourself:
- Keep `OATHWALL_GATEWAY_MIN_TOKENS` meaningful, and `RATE_PER_MIN` / `MAX_COMPLETION_TOKENS` conservative (defaults in `lib/core.mjs`).
...
- Rotating `OATHWALL_GATEWAY_SECRET` invalidates every issued token (your kill-switch).

## Honesty note

Call the *provider* "Oathwall AI" freely — white-labeling inference is normal. Just
