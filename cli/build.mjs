...
function skip(reason) {
  console.log(`[oathwall] skipping web build — ${reason}`);
  process.exit(0);
}

if (process.env.OATHWALL_SKIP_BUILD) skip("OATHWALL_SKIP_BUILD set");
...
console.log("[oathwall] building the dashboard (one-time, ~15s)…");
const res = spawnSync(process.execPath, [nextJs, "build"], {
  cwd: WEB,
  stdio: "inherit",
});
if (res.status !== 0) {
  console.log("[oathwall] web build failed — the CLI still works; run `npm run build` to retry.");
}
process.exit(0);
