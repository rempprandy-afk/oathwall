...
process.env.OATHWALL_HOME = path.join(process.cwd(), ".test-orch-home");
process.env.OATHWALL_HOSTED = "1";
process.env.OATHWALL_BUNDLER_API_KEY = "house-bundler-key";
process.env.GROQ_API_KEY = "house-groq-key";
process.env.OATHWALL_STORE_DEK = "SECRET-dek-never-to-a-child";
process.env.OATHWALL_SESSION_SECRET = "SECRET-session-never-to-a-child";
...
describe("orchestrator env curation", () => {
  it("childHome is per-tenant, lowercased, under children/", () => {
    assert.equal(childHome(T), path.join(process.env.OATHWALL_HOME!, "children", T.toLowerCase()));
  });
  ...
  it("childEnv injects the house keys but STRIPS the orchestrator-only secrets", () => {
    ...
    assert.equal(env.OATHWALL_BUNDLER_API_KEY, "house-bundler-key");
    assert.equal(env.GROQ_API_KEY, "house-groq-key");
    // per-child steering
    assert.equal(env.OATHWALL_HOSTED, "1");
    assert.equal(env.OATHWALL_HOME, childHome(T));
    // the three a child must NEVER see
    assert.equal(env.OATHWALL_STORE_DEK, undefined, "the DEK decrypts every tenant's key");
    assert.equal(env.OATHWALL_SESSION_SECRET, undefined, "the secret forges any session");
  ...
  it("fleetHaltFile sits under the orchestrator home", () => {
    assert.equal(fleetHaltFile(), path.join(process.env.OATHWALL_HOME!, "FLEET_HALT"));
  });
});
